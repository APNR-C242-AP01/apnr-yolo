...
def enhance_plate_image(plate_img):
    ...
    # Denoising to remove noise that can confuse OCR
    ...
    # Morphological operations for better contour detection
...
def read_plate(reader, plate_img, threshold=0.5):
    ...
    for img in attempts:
        ...
        if results:
            for (bbox, text, conf) in results:
                if conf >= threshold:  # Only accept OCR results with high confidence
                    all_texts.append((text, conf))
            
            if len(results) > 1:
                results.sort(key=lambda x: x[0][0][0])  # Sort based on bounding box position
                combined_text = ''.join(r[1] for r in results)
                avg_conf = sum(r[2] for r in results) / len(results)
                all_texts.append((combined_text, avg_conf))
    
    all_texts.sort(key=lambda x: x[1], reverse=True)  # Sort texts based on confidence
